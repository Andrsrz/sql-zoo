...
SELECT actor.name
FROM actor LEFT JOIN casting ON casting.actorid = actor.id
WHERE movieid=11768;
...
SELECT movie.title
FROM movie LEFT JOIN casting ON casting.movieid = movie.id
...
SELECT movie.title, actor.name
FROM movie LEFT JOIN casting ON (casting.movieid = movie.id AND casting.ord = 1)
LEFT JOIN actor ON (actor.id = casting.actorid)
...
WHERE (casting.movieid = 10095 OR casting.movieid = 11434 OR casting.movieid = 13630)
